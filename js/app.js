 ...
 */
    ...
    (() => {
        ...
        if (document.querySelector(".other-event__slider")) new core(".other-event__slider", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 3,
        ...
        if (document.querySelector(".main-slider-products__slider")) new core(".main-slider-products__slider", {
        ...
        if (document.querySelector(".product-card__slider")) new core(".product-card__slider", {
            modules: [ Pagination ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
...
***************************************************************************** */
        ...
        if (filters) {
            ...
            const checkboxesSelector = `${filters} .spollers__checkboxes`;
            ...
            if (checkboxesSelector) {
                const blocks = Array.from(document.querySelectorAll(".filter-column")).map((block => ({
                    node: block,
                    themes: Array.from(block.classList).filter((blockClass => blockClass.includes("filter__checkbox_")))
                })));
                const inputs = Array.from(document.querySelectorAll(".checkbox__input")).map((check => ({
                    node: check,
                    theme: check.getAttribute("data-filter")
                })));
                function onCheckBoxChange() {
                    const checkedInput = inputs.filter((c => c.node.checked));
                    if (0 == checkedInput.length) blocks.forEach((block => {
                        ...
                        console.log(buffBlocks);
                        checkedInput.forEach((input => {
                            buffBlocks = buffBlocks.filter((block => block.themes.includes(`filter__checkbox_${input.theme}`)));
                        }));
                        buffBlocks.forEach((block => block.node.classList.remove("_hidden-checkbox")));
                    }
                }
                document.querySelectorAll(".filter__spollers").forEach((n => {
        ...
        if (upButton) {
            ...
            function scrollFunction() {
                if (document.body.scrollTop > 20 || document.documentElement.scrollTop > 20) document.getElementById("myBtn").style.display = "flex"; else document.getElementById("myBtn").style.display = "none";
            }
