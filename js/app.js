    ...
    if (filters) {
        ...
        if (checkboxesSelector) {
            document.querySelectorAll(checkboxSelector).forEach((n => {
                n.addEventListener("change", onFilterButtonClick);
                n.dispatchEvent(new Event("change"));
            }));
            function onFilterButtonClick({currentTarget: {dataset: {filter}}}) {
                const activeItemClass = checkboxFilterClassPrefix + filter;
                this.closest(checkboxesSelector).querySelectorAll(checkboxSelector).forEach((n => {
                    n.classList.toggle(buttonActiveClass, n === this);
                }));
                this.closest(filters).querySelectorAll(itemSelector).forEach((function(n) {
                    n.classList.toggle(itemCheckboxHiddenClass, !n.classList.contains(activeItemClass));
                }));
            }
        }
    }
