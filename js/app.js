(() => {
    ...
    if (filters) {
        ...
        const checkboxSelector = `${filters} .filter__checkboxes`;
        const filterSelect = `${filters} .filter__select`;
        const buttonSelector = `${buttonsSelector} [data-filter]`;
        const selectSelector = `${filterSelect} [data-filter]`;
        ...
        const itemSelectHiddenClass = "_hidden";
        const itemFilterClassPrefix = "filter__column_";
        const selectFilterClassPrefix = "filter-select_";
        const filterReset = document.querySelector(".filter-reset span");
        if (buttonsSelector) {
        ...
        if (checkboxSelector) document.querySelectorAll(checkboxSelector).forEach((n => {
            n.addEventListener("change", (function() {
                document.querySelectorAll(itemSelector).forEach((function(n) {
                    n.classList.toggle("_hidden-checkbox", this.length && !this.includes(n.dataset.doctor));
                }), Array.from(this.querySelectorAll(":checked"), (n => n.dataset.filter)));
            }));
            n.dispatchEvent(new Event("change"));
        }));
        if (filterSelect) {
            document.querySelectorAll(selectSelector).forEach((n => {
                n.addEventListener("click", onFilterButtonClick);
            }));
            function onFilterButtonClick({currentTarget: {dataset: {filter}}}) {
                const activeItemClass = selectFilterClassPrefix + filter;
                this.closest(filterSelect).querySelectorAll(selectSelector).forEach((n => {
                    n.classList.toggle(buttonActiveClass, n === this);
                }));
                this.closest(filters).querySelectorAll(itemSelector).forEach((({classList: cl}) => {
                    cl.toggle(itemSelectHiddenClass, "all" !== filter && !cl.contains(activeItemClass));
                }));
            }
        }
        if (filterReset) filterReset.addEventListener("click", (function(e) {
            ...
            document.querySelectorAll(buttonSelector).forEach((e => {
                e.classList.remove("_active");
            }));
            document.querySelectorAll(selectSelector).forEach((e => {
                e.classList.remove("_active");
            }));
        }));
    }
